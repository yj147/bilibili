...
def _normalize_developer_name(value: object, fallback: str = "-") -> str:
    """Normalize legacy developer strings to plain name.

    Supports:
    - "Bambi" -> "Bambi"
    - "name=Bambi\\ninitialized_at=..." -> "Bambi"
    """
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    assignee = _normalize_developer_name(assignee, "")
    if not assignee:
        print(colored("Error: Invalid assignee value", Colors.RED), file=sys.stderr)
        return 1

    # Get current developer as creator
    creator = _normalize_developer_name(get_developer(repo_root), "") or assignee
        ...
        for d in sorted(tasks_dir.iterdir()):
            ...
            if task_json.is_file():
                ...
                if data:
                    ...
                    assignee = _normalize_developer_name(data.get("assignee", "-"))
